...
func TestNewOutgoing(t *testing.T) {
	...
	tests := []struct {
		...
		{"whisper message", args{&Incoming{
			...
			Channel:  "",
			Message:  "Hi!!",
			User:     &User{2, "turtoise", 485693696, "2020-06-24 06:01:01"},
			Raw:      nil,
			DMs:      true,
		}, "yo"}, &Outgoing{
			Platform: Twitch,
...
func TestFakeIncoming(t *testing.T) {
	...
	tests := []struct {
		...
		{"normal message", args{"quinndt", "squadR turdoise", &User{2, "turtoise", 485693696, "2020-06-24 06:01:01"}, false, Twitch}, &Incoming{
			Platform: Twitch,
			Channel:  "quinndt",
